...
const Menu = ({callback, links, handleBlur, y, menuOffset}) => {
    ...
    useEffect(()=>{
        //if(pageHasRendered.current){
            setMenu(
                // <div role="menu" className="library-submenu-right" style={{top : `${y - menuOffset - cssBorder}px`}}>
                <div role="menu" className="library-submenu-right" >
                    {
                        links.data.map((data, index) => {
                            if (data.role === 'menuitem'){
                                return (<p key={index} role="menuitem" className="library-menuitem"><Link to={data.input} onClick={closeMenu}>{data.title}</Link></p>)
                            }  else if (data.role === 'menu'){
                                return (<div className="library-menulink-group">
                                    <span className="library-menufronticon"></span><span className="library-menulink" key={index} role="menu" onClick={(event)=>{openSubmenu(data, index, event)}}><a>{data.title}</a></span><span className="library-menurearicon"></span>
                                </div>)
                            } else if (data.role === 'function'){
                                return (<p key={index} role="menuitem" className="library-menuitem"><a onClick={(evt)=>{closeMenu(evt) ; data.input();}}>{data.title}</a></p>)
                                ...
                                return (<p key={index} role="menuitem" className="library-menuitem"><a onClick={(evt)=>{closeMenu(evt) ; links.func(data.title, data.description, data.input, data.props);}}>{data.title}</a></p>)
