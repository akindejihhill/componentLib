...
const MainMenu = ({callback, links, icon}) => {
    ...
    const menuButton = (
        <div id="library-menu-button" tabIndex="1" onClick={toggleMainMenu}>
            <img className="library-menu-icon" src={icon} alt="Buttons"/>
        </div>
    );
    ...
    const handleBlur = (event) => {
        ...
        if(event.target.parentElement.className === "library-menuitem"){
            setUMVisibility(false);
            setMainMenu(null);
        }
    };
    ...
    useEffect(()=>{
        if(pageHasRendered.current){
            if(uMVisibility){
                setMainMenu(
                    <div role="menu" className="library-main-menu" onClick={(evt)=>{menuRef.current = evt.target;}}>
                        {
                            links.data.map((data, index) => {
                                if (data.role === 'menuitem'){
                                    return (<p key={index} role="menuitem" className="library-menuitem"><Link to={data.input} onClick={(evt)=>{setUMVisibility(false); setSMVisibility(false)}}>{data.title}</Link></p>)
                                } else if (data.role === 'menu'){
                                    return (<div className="library-menulink-group">
                                                <span className="library-menufronticon"></span><span className="library-menulink" key={index} role="menu"  onClick={(event)=>{toggleSubmenu(data, index, event)}}><a>{data.title}</a></span><span className="library-menurearicon"></span>
                                            </div>
                                        )
                                } else if (data.role === 'function'){
                                    return (<p key={index} role='menuitem' className="library-menuitem"><a onClick={(evt)=>{setUMVisibility(false); setSMVisibility(false); data.input();}}>{data.title}</a></p>)
                                        ...
                                        return (<p key={index} role="menuitem" className="library-menuitem"><a onClick={(evt)=>{setUMVisibility(false); setSMVisibility(false) ; links.func(data.title, data.description, data.input, data.props);}}>{data.title}</a></p>)
